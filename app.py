    ...
    if 'Preco' not in df.columns or 'Area_Construida' not in df.columns:
        ...
        for var in variaveis_independentes:
            if var == 'Area_Construida':
                ...
                caracteristicas_avaliando[var] = float(st.sidebar.slider(f"Quantidade de {label_nome}", 0, 5, 1 if var != 'Quartos' else 3))
            elif var == 'Conservacao':
                caracteristicas_avaliando[var] = float(st.sidebar.number_input("Estado de Conservação (1 a 3)", min_value=1.0, max_value=3.0, value=2.0, step=1.0))
            elif var == 'Padrao_Acabamento':
                caracteristicas_avaliando[var] = float(st.sidebar.number_input("Padrão de Acabamento (1 a 3)", min_value=1.0, max_value=3.0, value=2.0, step=1.0))
        ...
        if len(df) >= len(variaveis_independentes) + 2:
            ...
            # Construção estritamente segura da lista de valores do avaliando
            dados_imovel_lista = []
            for var in variaveis_independentes:
                dados_imovel_lista.append(float(caracteristicas_avaliando[var]))
                
            dados_imovel_array = np.array(dados_imovel_lista).reshape(1, -1)
            dados_imovel_scaled = scaler.transform(
